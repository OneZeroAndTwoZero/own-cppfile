...
void add(int pos,long long val){ // add val
    tree[pos].val = (tree[pos].val + (tree[pos].r - tree[pos].l + 1) * val);
    tree[pos].tag = (tree[pos].tag + val);
}
...
long long query(int pos,int l,int r){
    ...
    if(l <= mid){
        res = (res + query(2 * pos,l,r));
    }
    if(r >= mid + 1){
        res = (res + query(2 * pos + 1,l,r));
    }
    return res;
}
